...
export class SubjectComponent implements OnInit {
  ...
  getSubject( val ){
    ...
    console.log( "LIST Fired", val );
    let data = <SEARCH_QUERY_DATA>{};
        data.fields = 'idx, content, subject, category, varchar_1, varchar_2, varchar_3';
        data.from = "sf_post_data";
        data.where = "post_id='job' AND category='OES' AND subject='subject' AND varchar_2 in('" + val +"')";
        data.orderby = "idx asc";
      this.post.search( data, categoryData =>{
        this.subject_data = categoryData.search;
  ...
  filteringCategory( val ){
        ...
        this.filtered_category = _.join( temp , "','");

        this.getSubject( _.join( temp , "','") );
  }
